...
function IdeeCard({ idea, onChat }: { idea: Idee; onChat(m: string): void }) {
    ...
    <div className="rounded-2xl p-4" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
      ...
      <div className="flex gap-2">
        <a href={canvaLink(idea.format, idea.titre)} target="_blank" rel="noopener noreferrer"
          ...
          style={{ background: "linear-gradient(135deg,#fe2c55,#25f4ee20)", color: "white", border: "1px solid #fe2c5540" }}>
          🎨 Canva
        </a>
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid #fe2c5530", background: "#0d0d14" }}>
      ...
      <div className="p-4 space-y-3">
        {p.hook && <div>
          <p className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: "var(--muted)" }}>Hook</p>
          <p className="text-sm text-white italic">"{p.hook}"</p>
        </div>}
        {p.caption && <div>
          <p className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: "var(--muted)" }}>Caption</p>
          <p className="text-xs leading-relaxed" style={{ color: "#c0c0d8" }}>{p.caption}</p>
        </div>}
        {p.hashtags && <div>
          <p className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: "var(--muted)" }}>Hashtags</p>
          <p className="text-xs leading-relaxed" style={{ color: "#25f4ee" }}>{p.hashtags}</p>
        </div>}
        <div className="flex flex-wrap gap-2 pt-1">
          {cl && <a href={cl} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold"
            style={{ background: "linear-gradient(135deg,#fe2c55,#ff6b6b)", color: "white" }}>
            🎨 Créer sur Canva
          </a>}
          {gl && <a href={gl} target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold"
            style={{ background: "#1a1a28", color: "var(--muted)", border: "1px solid var(--border)" }}>
            📅 Planifier
          </a>}
          {p.hashtags && <button onClick={copy}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold"
            style={{ background: "#1a1a28", color: copied ? "#25f4ee" : "var(--muted)", border: "1px solid var(--border)" }}>
...
export default function TrendaApp() {
  ...
  const send = useCallback(async (text: string) => {
    ...
    } catch { setMsgs(prev => { const c = [...prev]; c[c.length - 1] = { role: "assistant", content: "⚠️ Erreur. Réessaie." }; return c; }); }
  ...
  const Tendances = () => {
    if (loading) return (
      <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6">
        <div className="w-20 h-20 rounded-3xl icon-glow flex items-center justify-center text-4xl"
        ...
        <div className="flex gap-1.5 mt-2">
          {[0,1,2].map(i => <span key={i} className="typing-dot w-2 h-2 rounded-full" style={{ background: "var(--tiktok-red)", animationDelay: `${i*.15}s` }} />)}
        </div>
      </div>
    );
    if (error || !data) return (
      <div className="flex-1 flex flex-col items-center justify-center gap-3 px-6">
        ...
        <p className="text-sm text-center" style={{ color: "var(--muted)" }}>Vérifie ta clé API Anthropic dans .env.local</p>
        <button onClick={() => loadTrends()} className="mt-2 px-5 py-2.5 rounded-2xl font-bold text-white text-sm"
          style={{ background: "linear-gradient(135deg,#fe2c55,#ff6b6b)" }}>Réessayer</button>
      </div>
    );
    return (
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-5 scrollbar-hide">
        ...
        <div className="p-4 rounded-2xl" style={{ background: "linear-gradient(135deg,#fe2c5515,#25f4ee10)", border: "1px solid #fe2c5530" }}>
          ...
          <button onClick={() => send("Sur la base des tendances actuelles, propose-moi 3 idées de contenu détaillées avec scripts, captions et planning.")}
        ...
        <section>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              <div className="w-1 h-4 rounded-full" style={{ background: "linear-gradient(#fe2c55,#ff8800)" }} />
              <p className="text-xs font-bold uppercase tracking-wider" style={{ color: "var(--muted)" }}>Sons en tendance</p>
            </div>
        ...
        <section>
          <div className="flex items-center gap-2 mb-2">
            <div className="w-1 h-4 rounded-full" style={{ background: "linear-gradient(#25f4ee,#8b5cf6)" }} />
            <p className="text-xs font-bold uppercase tracking-wider" style={{ color: "var(--muted)" }}>Hashtags qui montent</p>
          </div>
        ...
        <section>
          <div className="flex items-center gap-2 mb-2">
            <div className="w-1 h-4 rounded-full" style={{ background: "linear-gradient(#10b981,#25f4ee)" }} />
            <p className="text-xs font-bold uppercase tracking-wider" style={{ color: "var(--muted)" }}>Idées de contenu</p>
          </div>
          <div className="space-y-2">{data.idees.map((idea, i) => <IdeeCard key={i} idea={idea} onChat={send} />)}</div>
        </section>
        ...
        {data.peak_times.length > 0 && (
          <section>
            <div className="flex items-center gap-2 mb-2">
              <div className="w-1 h-4 rounded-full" style={{ background: "linear-gradient(#f59e0b,#fe2c55)" }} />
              <p className="text-xs font-bold uppercase tracking-wider" style={{ color: "var(--muted)" }}>Heures de forte affluence</p>
            </div>
              ...
              {data.peak_times.map((pt, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: "#1a1a28" }}>
                    <div className="h-full rounded-full" style={{ width: `${pt.score}%`, background: pt.score > 80 ? "linear-gradient(90deg,#fe2c55,#f59e0b)" : pt.score > 60 ? "#f59e0b" : "#25f4ee", transition: "width .6s ease" }} />
                  </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3 scrollbar-hide">
        {msgs.length === 0 && (
          <div className="py-10 px-2 text-center">
            <div className="w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center text-3xl icon-glow"
              style={{ background: "linear-gradient(135deg,#fe2c55,#25f4ee)" }}>📱</div>
            <p className="font-bold text-white mb-1.5">Trenda est prête</p>
            <p className="text-sm mb-5" style={{ color: "var(--muted)" }}>Dis-moi ce que tu veux créer ou pose une question sur les tendances.</p>
                      ...
                      : <div className="px-4 py-2.5 rounded-2xl rounded-tl-sm text-sm whitespace-pre-wrap leading-relaxed"
                          ...
                          {m.content || (streaming && i === msgs.length - 1
                            ? <span className="inline-flex gap-1">
                                {[0,1,2].map(j => <span key={j} className="typing-dot w-1.5 h-1.5 rounded-full" style={{ background: "#fe2c55", animationDelay: `${j*.15}s` }} />)}
                              </span>
                            : null)}
                        </div>
                    }
      ...
      {!data
        ...
        : <div className="space-y-3">
            <div className="flex items-center justify-between mb-1">
              ...
              <button onClick={() => send("Génère un planning TikTok sur 7 jours avec des horaires selon les heures de forte affluence. Adapte au marché francophone.")}
            ...
            <button onClick={() => { setTab("chat"); send("Je veux un planning personnalisé selon mes disponibilités. Pose-moi quelques questions."); }}
      ...
      <div className="shrink-0 px-4 pb-3"
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <button onClick={() => setShowNiche(true)}
            ...
            <span className="text-xs font-semibold" style={{ color: "var(--muted)" }}>{niche || "Ma niche"}</span>
          </button>
        ...
        <div className="flex gap-1 p-1 rounded-2xl" style={{ background: "var(--card)" }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              style={{
                background: tab === t.id ? "linear-gradient(135deg,#fe2c55,#ff6b6b)" : "transparent",
        ...
        {showNiche && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.form initial={{ y: 60 }} animate={{ y: 0 }} exit={{ y: 60 }}
              ...
              <p className="text-sm" style={{ color: "var(--muted)" }}>Trenda adapte toutes ses suggestions à ta niche pour des tendances ultra-ciblées.</p>
